...
def record_audio(rec_rcv,
    ...
    while True:
        if cmd_rcv.poll():
            ...
            logger.info('dump args:%s', dump_args)
            ...
            logger.info('trying to write:%s', dump_fname)
            np.asarray(
                _buffer, dtype=np.int16)[-dump_size * channels * rate:].tofile(dump_fname)
            ...
            data = emb_rcv.recv()
            _emb_buffer = np.concatenate(
                (_emb_buffer, data.reshape(1, -1)), axis=0)
